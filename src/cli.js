...
${c.dim}Scanned by halfday-env-scan • halfday.dev${c.reset}
`);
}
...
async function main() {
  ...
  if (positional.length > 0) {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        if (jsonMode) {
          console.log(JSON.stringify({ error: `File not found: ${source}` }));
        } else {
          console.error(`${c.red}Error:${c.reset} File not found: ${source}`);
        }
        process.exit(1);
      }
      throw err;
  ...
  if (!findings) {
    if (jsonMode) {
      console.log(JSON.stringify({ source, findings: [], grade: 'A', label: 'Excellent', pass: true }));
    } else if (!quietMode) {
      console.log(`${c.dim}Empty input — nothing to scan.${c.reset}`);
    }
    process.exit(0);
  }
  ...
  if (quietMode) {
    const gc = gradeColor[score.grade] || c.white;
  ...
  console.log();
  console.log(`  ${c.dim}Scanned by halfday-env-scan • halfday.dev${c.reset}`);
  console.log();
